run:
    ...
    #
    # -tags dev is what compiles LONGBOX_DEV_AUTH in at all; without it the
    # variable below is inert and the dev server asks for a passkey. The bypass
    # is behind a build tag so no release binary can carry it. LONGBOX_ADDR
    ...
    LONGBOX_DB=./longbox.db LONGBOX_LIBRARY=./library-dev LONGBOX_DATA=./data-dev \
        LONGBOX_ADDR=127.0.0.1:8080 \
        LONGBOX_ORIGIN=http://localhost:5173 \
        LONGBOX_DEV_AUTH=dev go run -tags dev ./cmd/longbox 2>&1 | prefix api &
