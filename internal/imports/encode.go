...
// goodEnoughExt are the source formats an encode pass copies through untouched.
// AVIF and WebP are already space-efficient, so re-encoding them would spend CPU
// and a generation of quality to save little or nothing; a page already in one
// of these is kept as-is regardless of the target format, which is also why a
// target may never be one that would want to re-encode these.
