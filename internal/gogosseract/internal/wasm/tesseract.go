...
import (
	...
	"github.com/SeriousBug/dowitcher/internal/gogosseract/internal/gen"
...
func CompileTesseract(ctx context.Context, waRT wazero.Runtime, embEng embind.Engine, cfg CompileConfig) (api.Module, error) {
	patchedWASM, err := tesseractWASMPatched()
	if err != nil {
		return nil, errors.Errorf("addStackExportAliases %w", err)
	}
	tessCompiled, err := waRT.CompileModule(ctx, patchedWASM)
	if err != nil {
